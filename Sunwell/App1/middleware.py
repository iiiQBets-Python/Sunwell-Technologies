...
class UserActivityLoggingMiddleware:
    def __init__(self, get_response):
        ...
        post_delete.connect(
            ...
            dispatch_uid="log_delete_action")

    def __call__(self, request):
        # Store the current request in thread-local storage
        _thread_locals.request = request

        # Get logged-in user or default to 'Anonymous'
    ...
    def log_exception(self, request, exception, emp_user):
        ...
        error_logger.error(
            f"{emp_user} is getting {
                type(exception).__name__} on accessing {view_name}: {
                str(exception)}"
        )
    ...
    def log_save_action(sender, instance, created, **kwargs):
        ...
        emp_user = request.session.get(
            'username', 'Anonymous') if request else 'Anonymous'
